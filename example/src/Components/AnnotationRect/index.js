...
class AnnotationRect{
    constructor(data) {
      ...
      this.sameKeyAnnotations = data?.sameKeyAnnotations
      ...
      this.style = {
        ...
        borderColor:
          data?.style?.borderColor ?? this.calcBorderColor(this.confidence),
      ...
      const widthSize =
        fabric.Textbox.prototype.fontSize -
        this.w / this.text.split('\n')[0].length
      const heightSize =
        fabric.Textbox.prototype.fontSize -
        this.h / (widthSize * this.text.split('\n').length)
      this.coords = this.calcTextPosition(
        this.canvas,
        this.x,
        this.y,
        this.w,
        this.h,
        (widthSize + heightSize) / 2 + 2,
      ...
      fabric.Textbox.prototype.cacheProperties =
        fabric.Textbox.prototype.cacheProperties.concat('active')
      ...
      //   this.labelText = new fabric.Text(
      //     this.generateLabelText(this.parent, this.field_name, this.confidence),
      //     this.labelOptions
      //   )
      this.sameKeyAnnotations.forEach((ann) => {
        this.labelText = new fabric.Text(
          this.generateLabelText(this.parent, ann.field_name, ann.confidence),
          this.labelOptions
        )
        this.canvas.add(this.labelText)
        this.labelText.width = this.w
        this.canvas.renderAll()
        this.labelOptions = {
          ...this.labelOptions,
          top:
            this.y -
            this.confidenceFontSize -
            this.style.strokeWidth -
      ...
      this.verticalBar.set(
        'height',
        this.rect.height +
          this.textBox.height +
          this.labelText.height +
          this.textBox.fontSize / 5
      )
